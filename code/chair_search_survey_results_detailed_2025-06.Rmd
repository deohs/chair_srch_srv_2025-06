...
pacman::p_load(here, folders, readxl, tibble, dplyr, tidyr, stringr, purrr, 
               vote, ggplot2, colorspace, forcats, CGPfunctions)
```
...
```{r q04_affect_start_xtab_myrank_plot, fig.height=3.5}
PlotXTabs2(df, x = q04_affect_start, y = q01_myrank, results.subtitle = FALSE, 
           title = 'q04_affect_start by respondent rank', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') +
  coord_flip()
```

```{r q04_affect_start_xtab_mytrack_plot, fig.height=3.5}
PlotXTabs2(df, x = q04_affect_start, y = q02_mytrack, results.subtitle = FALSE,  
           title = 'q04_affect_start by respondent track', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') + 
  coord_flip()
```
...
```{r q06_affect_cost_xtab_myrank_plot, fig.height=3.5}
PlotXTabs2(df, x = q06_affect_cost, y = q01_myrank,  results.subtitle = FALSE, 
           title = 'q06_affect_cost by respondent rank', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') +
  coord_flip()
```

```{r q06_affect_cost_xtab_mytrack_plot, fig.height=3.5}
PlotXTabs2(df, x = q06_affect_cost, y = q02_mytrack,  results.subtitle = FALSE, 
           title = 'q06_affect_cost by respondent track', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') + 
  coord_flip()
```
...
```{r q08_affect_funds_xtab_myrank_plot, fig.height=3.5}
PlotXTabs2(df, x = q08_affect_funds, y = q01_myrank,  results.subtitle = FALSE, 
           title = 'q08_affect_funds by respondent rank', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') +
  coord_flip()
```

```{r q08_affect_funds_xtab_mytrack_plot, fig.height=3.5}
PlotXTabs2(df, x = q08_affect_funds, y = q02_mytrack,  results.subtitle = FALSE, 
           title = 'q08_affect_funds by respondent track', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') + 
  coord_flip()
```
...
```{r q10_affect_freeze_xtab_myrank_plot, fig.height=3.5}
PlotXTabs2(df, x = q10_affect_freeze, y = q01_myrank,  results.subtitle = FALSE, 
           title = 'q10_affect_freeze by respondent rank', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') +
  coord_flip()
```

```{r q10_affect_freeze_xtab_mytrack_plot, fig.height=3.5}
PlotXTabs2(df, x = q10_affect_freeze, y = q02_mytrack,  results.subtitle = FALSE, 
           title = 'q10_affect_freeze by respondent track', xlab = '',
           sample.size.label = FALSE, legend.position = 'bottom') + 
  coord_flip()
```
...
df_vote$Rank[1:5] <- rank(df_vote$Total[1:5], ties.method='min')
...
df_vote_score <- df_rank_wide %>% count.votes(
    method = 'score', max.score = 5, larger.wins = FALSE, quiet = TRUE
    ) %>% summary() %>% rownames_to_column('Rank') %>% 
  mutate(Total = as.integer(Total)) %>% as_tibble()
df_vote_score$Rank[1:5] <- rank(df_vote_score$Total[1:5], ties.method='min')
df_vote_score %>% knitr::kable(align = 'llrc')
```

The two methods yielded identical results: `r identical(df_vote, df_vote_score)`

```{r q12_ranking_plot, fig.height=2.5}
df_rank <- df_rank %>%  
...
df_vote[1:5, ] %>% 
  mutate(Candidate = factor(Candidate, ordered = TRUE, 
                            levels = levels(df_rank$Candidate))) %>% show_bar_plot_identity(x = Candidate, y = Total, title = title)
```
...
df_rank %>% 
  ...
  geom_text(aes(label = Rank), alpha = 0.5, color = pal_text, size = 4) + 
