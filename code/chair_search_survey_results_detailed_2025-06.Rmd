...
knitr::opts_chunk$set(echo = FALSE, cache.rebuild = TRUE)
```
...
show_xtab_plot <- function(df, x, y, title.y) {
  df[, x] <- fct_rev(df[, x]) 
  PlotXTabs2(df, any_of(x), any_of(y), results.subtitle = FALSE, 
             legend.title = y, title = paste(x, 'by', title.y), xlab = '',
