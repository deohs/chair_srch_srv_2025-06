...
xform_ranking <- function(x, condorcet.ranking = FALSE) {
  df <- x %>% 
...
rank_ties <- function(df, rankvar = 'Rank', valvar = 'Total', rows = 1:5, 
  ...
  if (isTRUE(invert)) {
    df[[rankvar]][rows] <- 
      as.numeric(fct_rev(factor(rank(df[[valvar]][rows], ties.method='min'))))
  } else {
    df[[rankvar]][rows] <- 
...
Using `condorcet()` will elect the candidate that beats all other candidates in 
pairwise comparisons. The Condorcet loser will be the candidate that loses 
against all other candidates. It is possible that neither a Condorcet winner or 
loser exists. In this case, they do, and the winner is the same as with the 
previous methods shown above.

```{r q12_condorcet_vote}
df_rank <- map_df(df$q12_ranking, xform_ranking, condorcet.ranking = TRUE, 
                  .id = 'id')
df_rank_wide <- df_rank %>% 
  pivot_wider(names_from = Candidate, values_from = Rank, values_fill = NA) %>%
  select(-id)
