...
pacman::p_load(here, folders, readxl, tibble, dplyr, tidyr, stringr, purrr, 
               vote, ggplot2, colorspace, forcats)
```
...
show_bar_plot <- function(df, x, lv, title = '') {
  df %>% mutate(fct = factor({{x}}, ordered = TRUE, levels = rev(lv))) %>% 
    ggplot(aes(fct)) + geom_bar() + coord_flip() + 
...
show_bar_plot_identity <- function(df, x, y, lv, title = '') {
  df %>% mutate(fct = factor({{x}}, ordered = TRUE, levels = rev(lv))) %>% 
    ggplot(aes(fct, {{y}})) + geom_bar(stat = 'identity') + coord_flip() +
...
show_bar_plot(df, q04_affect_start, level, 'q04_affect_start')
```
...
show_bar_plot(df, q06_affect_cost, level, 'q06_affect_cost')
```
...
show_bar_plot(df, q08_affect_funds, level, 'q08_affect_funds')
```
...
show_bar_plot(df, q10_affect_freeze, level, 'q10_affect_freeze')
```
...
show_bar_plot_identity(df = df_vote[1:5, ], x = Candidate, y = Total, 
                       lv = levels(df_rank$Candidate), title = title)
```
